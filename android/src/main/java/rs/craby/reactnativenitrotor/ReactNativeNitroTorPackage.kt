package rs.craby.reactnativenitrotor
...
class ReactNativeNitroTorPackage : BaseReactPackage() {
  companion object {
    val JNI_PREPARE_MODULE_NAME = setOf(
      "__crabyReactNativeNitroTor_JNI_prepare__"
    )
  }

  init {
    SoLoader.loadLibrary("cxx-react-native-nitro-tor")
  }

  override fun getModule(name: String, reactContext: ReactApplicationContext): NativeModule? {
    if (name in JNI_PREPARE_MODULE_NAME) {
      nativeSetDataPath(reactContext.filesDir.absolutePath)
      return ReactNativeNitroTorPackage.TurboModulePlaceholder(reactContext, name)
    }
    return null
  }
