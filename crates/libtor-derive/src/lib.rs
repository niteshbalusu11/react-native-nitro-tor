...
fn generate_test(
    ...
    quote_spanned! {span=>
        ...
        fn #test_name() {
            ...
            let expanded_cli = match expanded.as_slice() {
                [flag] => flag.clone(),
                [flag, value] => format!("{} \"{}\"", flag, value),
                _ => panic!("a Tor flag must expand to one or two arguments"),
            };
            println!("{:?} => {}", v, expanded_cli);
            assert_eq!(expanded_cli, #expected);
        }
    }
}
